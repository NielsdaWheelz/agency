...
runner ids are capability-based (`claude-code`, `codex`, `amp`, `opencode`, `cursor`, `droid`) and runner commands must be explicitly configured in user config (`config.runners`). legacy `claude` input aliases to `claude-code`.
