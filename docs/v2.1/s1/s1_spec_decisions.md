...
| D-002 | How should Gate A/B sets be sourced to avoid drift? | Gate sets are exactly those in `constitution.md` sections A/B; drift against `constitution.md` is treated as contract error. | Keeps one canonical source while enforcing consistency checks. | none | `@nnandal` + `Codex` | fixed in L2 |
...
| D-007 | How do we prevent release-gate list drift across docs? | Treat `constitution.md` as canonical membership and require synchronization validation against `constitution.md` from canonical sources. | Keeps validation enforceable in machine-checkable API flows while preventing ambiguous gate scope. | none | `@nnandal` + `Codex` | fixed in L2 |
