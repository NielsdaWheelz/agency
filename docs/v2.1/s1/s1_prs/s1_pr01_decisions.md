...
|---|---|---|---|---|---|---|---|---|
| PR01-D-001 | Which document defines authoritative Gate A/B membership? | Use `docs/v2.1/constitution.md` Gate A and Gate B sections only. | Scanning `constitution.md` directly; merging multiple gate sources. | Preserves invariant that gate sets are exactly release-gates A/B. | Add parser tests for A/B extraction only and order stability. | none | `@nnandal` + `Codex` | fixed in PR-01 |
