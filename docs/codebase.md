...
- `internal/daemon/` owns daemon lifecycle, handlers, reconciliation, streaming, checkpointing, and landing flows.
- `internal/daemonclient/` owns the daemon transport client.
- `internal/store/` owns on-disk layout, schema types, scans, and load/save behavior.
...
- Daemon packages own mutating control-plane policy when a daemon surface exists.
