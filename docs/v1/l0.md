...
- stored in `~/.agency/repo_index.json` mapping repo_key → seen paths
...
- `AGENCY_LOG_DIR` — global logs path
...
- **retains** `~/.agency/repos/<repo_id>/runs/<run_id>/` (meta.json, logs/)

### retention

v1: archived metadata retained indefinitely. user can manually delete `~/.agency/repos/<repo_id>/runs/<run_id>/` to reclaim space. `agency gc` deferred to post-v1.
...
implementation: coarse repo-level lock file (`~/.agency/repos/<repo_id>/.lock`). if lock held, error `E_REPO_LOCKED`.
