...
note: `show` never requires being inside a repo; it resolves from ${AGENCY_DATA_DIR} without git. If cwd is inside a repo, a git check may be used only for best-effort repo_root display.
...
stdout/stderr contract (locked):
- `--json`: always write the JSON envelope to stdout, even on error
- any human error message goes to stderr
- exit code reflects success/failure as specified
...
for each run record (including broken where possible):
	•	worktree_present = os.Stat(meta.worktree_path) succeeds and isDir (if meta missing: worktree_present=false)
	•	archived = !worktree_present
	•	report_path = <worktree_root>/.agency/report.md
	•	report_exists = stat(report_path) succeeds and is regular file
	•	report_bytes = file size if exists else 0
	•	if worktree missing: report_exists=false, report_bytes=0
	•	tmux_active:
	•	true if tmux session agency:<run_id> exists, else false
	•	if tmux binary missing or tmux server not running: tmux_active=false for all runs (no error)
	•	no tmux warnings in --json or --path modes
...
- title
- runner
- created_at (absolute timestamp; RFC3339)
- repo_id
- repo_key (if known)
- origin_url (if known)
...
- pr_number
- pr_url
- last_push_at (RFC3339 if present)
...
- if tmux unavailable: warn “tmux unavailable; tmux_active=false”

formatting constraints:
- keep it plain text, sections with labels
- no new table libs / deps
- use deterministic section order as listed above
...
- on ambiguous or not found: print nothing to stdout; error to stderr; non-zero exit
- on broken run: still print best-effort paths (with blanks where unknown), then exit non-zero E_RUN_BROKEN
	•	repo_root/worktree_root/report_path empty
	•	run_dir/events_path/transcript_path/logs_dir derived from run_dir
	•	logs_dir is a --path-only convenience; json exposes log paths under derived.logs
...
json rules:
	...
	•	on E_RUN_ID_AMBIGUOUS or E_RUN_NOT_FOUND: data=null (still emit envelope)
	•	logs paths must be present as strings even if files missing
	•	all paths must be absolute (where applicable) and deterministic
	•	errors must not write human text to stdout when --json is set
	•	json field names must use snake_case via explicit json tags (e.g., derived_status, worktree_present)
	•	timestamps are RFC3339 in json output
	•	created_at in human output is RFC3339, not relative
...
helper signature (locked):
	•	internal/paths/logs.go (or equivalent)
	•	func ResolveScriptLogPaths(runDir string) (setup, verify, archive string)
	•	resolution order:
	•	1) check known s1 filenames (fixed list)
	•	2) fall back to <run_dir>/logs/{setup,verify,archive}.log
	•	do not scan arbitrary files; only check a small fixed set
...
path mode
	...
	•	on broken run: still prints best-effort paths and exits non-zero E_RUN_BROKEN
...
unit tests
	...
	•	--path still prints best-effort paths and exits non-zero
...
test constraints:
	...
	•	when --json is set, stdout must be JSON only; errors must go to stderr
...
guardrails
	...
	•	do not add new deps
	•	do not add --capture
	•	do not run git commands except to check whether cwd is inside a repo for repo_root display; no other git operations
	•	error output must never be printed to stdout when --json or --path
