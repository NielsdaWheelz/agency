...
- update `repo.json` fields:
  ...
  - `origin_url` (if present; else omit or set empty string—match existing s0 contract)
...
this pr must ensure at minimum these fields exist and are updated correctly:
	...
	•	last_seen_at (utc rfc3339)
	•	origin_url (string, may be empty)
...
test cases:
	...
	•	create local branch main (or use whatever branch the test expects)
...
guardrails
	...
	•	keep changes scoped to the minimal packages needed (internal/git, internal/repo, etc.)
