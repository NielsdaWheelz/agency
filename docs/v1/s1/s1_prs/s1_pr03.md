...
load and validate `agency.json` at repo root for slice 01 needs, and resolve the **setup script** and **runner command** strings deterministically (no execution).
...
- locate repo root (reuse existing repo-root resolution helper; do not reimplement)
- read `agency.json` from repo root
...
- resolve the runner command string used by slice 01:
  - if `runners.<name>` exists: use its string value **verbatim**
  - else if `<name>` is exactly `claude` or `codex`: use literal `claude` / `codex`
...
notes:
	•	Verify/Archive fields may exist in the struct for forward compatibility but must not be validated as required in this pr.
