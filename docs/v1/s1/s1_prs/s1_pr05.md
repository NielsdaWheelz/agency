...
- surface failures as `AgencyError` with:
  ...
  - `details` includes:
    ...
    - stderr (or combined output) truncated only if enormous (keep full if reasonable)
...
then
	...
	•	sections exactly as slice spec expects (match S1 spec template; do not invent new sections)
	•	performs ignore check:
	•	if .agency/ not ignored: adds a warning string to state (no failure)
...
run with cwd = st.Repo.RepoRoot:
	•	git worktree add -b <branch> <worktree_path> <parent_branch>

ignore check

run with cwd anywhere (recommended cwd = st.Repo.RepoRoot) and pass -C <worktree>:
...
global state
	•	no global files written in this PR (meta/logs are PR-06/07)
...
test cases:
	...
	•	call CreateWorktree again is not expected (it would collide); instead:
	•	directly call the report creation helper (if you split it) OR
	•	create a second worktree in a different run_id and pre-create report file before invoking scaffolding helper
