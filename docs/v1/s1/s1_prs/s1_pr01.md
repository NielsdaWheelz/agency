# agency slice 01 — pr-01 spec: core utilities + errors + subprocess + atomic json

## goal
add the smallest reusable “core” layer needed by later s1 prs: run ids, slug/branch helpers, structured errors, subprocess runner, and atomic json writes.
...
- no “pipeline” or orchestration scaffolding
...
notes:
	•	rand4 is 4 lowercase hex chars derived from crypto/rand (2 bytes).
	•	now is provided by caller for testability; use now.UTC() for formatting.
...
important: this function returns the script, not "sh -lc '...'". later tmux code should execute:
	•	sh -lc <script> by passing args directly (no outer shell quoting).
...
printing policy (for later cli prs; implement now as helpers):
	•	Error() should include code + message, not dump details/cause by default.
	•	format exactly: "<Code>: <Message>"
	•	details/cause are for structured handling/logging later.
	•	NewError/WrapError must defensively copy Details (or nil if empty).
...
requirements:
	...
	•	if opts.Timeout > 0: derive ctx2, cancel := context.WithTimeout(ctx, opts.Timeout)
	•	stdin is always nil or strings.NewReader("")? no. must be /dev/null equivalent:
	•	set cmd.Stdin = nil (go stdlib reads from os.DevNull)
	•	do not inherit terminal stdin
	•	capture stdout/stderr buffers (strings ok)
	•	env behavior:
	•	start from os.Environ()
	•	apply EnvAdd overrides/additions
	•	exit code rules:
	•	if process exits with code N: return ExitCode=N, err=nil
	•	if command failed to start: return err != nil (wrap later), ExitCode=-1
	•	if context deadline exceeded: return ExitCode=124, err=nil (so callers can map to E_SCRIPT_TIMEOUT)
	•	if context canceled: return ExitCode=125, err=context.Canceled
	•	still include whatever stdout/stderr captured
...
// - rename over target
// - best-effort fsync dir (ok to skip on platforms where it’s painful)
...
when: NewRunID(now)
then:
	•	matches regex ^\d{14}-[0-9a-f]{4}$
	•	timestamp portion equals 20260109013207
	•	rand suffix is lowercase hex
...
when: RunCmd(ctx, “sh”, []string{”-lc”, “sleep 10”}, Timeout=50ms)
then:
	•	result.ExitCode == 124
	•	err == nil
...
when: RunCmd(ctx, “sh”, []string{”-lc”, “sleep 10”}, Timeout=0)
then:
	•	result.ExitCode == 125
...
when: RunCmd(ctx, “no_such_cmd”, nil, Timeout=0)
then:
	•	result.ExitCode == -1
	•	err != nil
...
unit tests (required)
	•	TestNewRunID_FormatAndUTC
	•	TestShortID
	•	TestSlugify_Table
	•	TestSlugify_TruncationDoesNotEndWithHyphen
	•	TestBranchName
	•	TestShellEscapePosix_Table
	•	TestShellEscapePosix_EmptyString
	•	TestShellEscapePosix_Newline
	•	TestBuildRunnerShellScript
	•	TestRunCmd_ExitCode
	•	TestRunCmd_TimeoutExit124
	•	TestRunCmd_CanceledExit125AndErrCanceled
	•	TestRunCmd_StartFailureExitMinus1AndErr
	•	TestWriteJSONAtomic_ReplacesAndIsValidJSON
	•	TestWriteJSONAtomic_PermApplied

tests must be deterministic:
	•	for run ids: pass fixed now; do not assert exact rand suffix, only format.
	•	for atomic write: write initial file, then overwrite with different json, ensure final file parses and equals new content.
...
guardrails
	•	stdlib only. do not add cobra/viper/bubbletea/loggers.
	•	do not add any cli commands or flags.
	•	do not write any repo/global state files (repo.json/meta.json). only helpers.
	•	do not invent new error codes here (just implement the error type).
	•	do not add concurrency/locks.
	•	keep everything under internal/core (or equivalent single package). no spread.

⸻

definition of done
	•	all new code is under internal/core/
	•	go test ./... passes
	•	no existing packages changed (except go.mod/go.sum if required)
	•	helpers match signatures + behaviors above exactly
