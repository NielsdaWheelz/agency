...
func ResolveDirs(env map[string]string, homeDir string) Dirs
...
behavior:
	...
	•	strings.TrimSpace
	•	if not absolute, join with cwd and filepath.Clean
	•	return absolute path (use filepath.Abs on the cleaned value)
...
behavior:
	...
	•	Present=true
	•	URL=trimmed stdout
	•	Host=best-effort parse host:
	•	ssh scp-like: git@host:owner/repo(.git)? -> host = part between @ and :
	•	https: https://host/owner/repo(.git)? -> host parsed by net/url
	•	otherwise Host=””
...
func DeriveRepoIdentity(absRepoRoot string, originURL string) (RepoIdentity, error)
...
hashing:
	...
	•	define constants:
	•	const RepoIDLen = 16 (or 12; pick one and lock it)
	•	repo_id = sha256(repo_key)[:RepoIDLen]
	•	path_hash = sha256(absRepoRoot) full hex (not truncated) inside repo_key OR (preferred) also truncate? choose and lock:
	•	recommendation: keep full hex inside path: repo_key to reduce collisions; it’s not user-facing.
...
unit tests (required)
	...
	•	weird formats:
	•	ssh://git@github.com/foo/bar.git -> Host=”” (unsupported in v1, so should not accidentally accept)
