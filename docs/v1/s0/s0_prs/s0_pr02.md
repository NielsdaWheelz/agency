...
- strict validation with crisp, stable error messages:
  ...
  - ignore unknown keys everywhere; fail only if a known key has the wrong type
...
behavior:
	...
	•	repoRoot is assumed to be a valid path; this PR does not verify it is a git repo.
...
validation rules (exact)
	•	version:
	•	required
	•	must be integer 1 (reject 1.0 or fractional values)
	•	defaults:
	•	required object
...
runner resolution:
	...
	•	cmd must be non-empty string and contain no whitespace; else error E_INVALID_AGENCY_JSON (runners.<name> must be a single executable (no args); use a wrapper script)
...
error stability note:
	•	define a small internal ValidationError { Field string; Msg string }
	•	AgencyError wraps ValidationError; FirstValidationError formats stable strings
...
add internal/config/testdata/:
	...
	•	empty_strings.json
	•	unknown_keys.json (extra keys at top level and inside defaults/scripts; should still validate)
