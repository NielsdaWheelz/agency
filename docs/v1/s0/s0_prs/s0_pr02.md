...
- strict validation with crisp, stable error messages:
  ...
  - `runners` values must be strings when present
...
validation rules (exact)
	...
	•	empty string values are invalid only if referenced by defaults.runner; other entries may be empty? (recommend: reject empty for any entry to avoid silent misconfig. do that.)

runner resolution:
	•	let name = defaults.runner
	•	if cfg.Runners != nil and cfg.Runners[name] exists:
	•	cmd must be non-empty string; else error E_INVALID_AGENCY_JSON (runners.<name> must be non-empty)
