...
rules:
	...
	•	repo_id is derived (sha256(repo_key) truncated) and stored redundantly
	•	paths:
...
rules:
	...
	•	true iff origin host == github.com and repo_key is github:…
	•	created_at set on first write, never changes
	•	last_seen_at updated on each successful doctor later (but this PR only provides API)
...
type RepoRecord struct {
  ...
  GitHubFlowAvailable bool   `json:"github_flow_available"`
  CreatedAt           string `json:"created_at"`
  LastSeenAt          string `json:"last_seen_at"`
}
...
behavior requirements:
	...
	•	if invalid json -> return E_STORE_CORRUPT (new code) with message including filename
	•	SaveRepoIndex:
	•	ensure ${AGENCY_DATA_DIR} exists (mkdirall)
	•	marshal with indentation (stable diffs / human readable) OR compact; pick one and lock it (recommend: indent with 2 spaces)
	•	atomic write via WriteFileAtomic
	•	UpsertRepoIndexEntry:
	•	create entry if missing; set created_at and last_seen_at = now
	•	always update last_seen_at = now
	•	update paths: insert absPath if missing; keep de-duped
	•	LoadRepoRecord:
	•	if missing -> (zero, false, nil)
	•	if invalid json -> E_STORE_CORRUPT
	•	BuildRepoRecord:
	•	if record existed previously, caller should preserve CreatedAt; this helper should set both created_at and last_seen_at to now (for first write). keep it simple:
	•	provide an additional helper MergeRepoRecord(existing *RepoRecord, updated RepoRecord) RepoRecord OR document that doctor will do it later.
	•	minimum in this PR: add a helper:

func (s *Store) UpsertRepoRecord(existing *RepoRecord, input BuildRepoRecordInput) RepoRecord

which:
	•	if existing != nil: preserves CreatedAt, updates LastSeenAt
...
unit tests (required)
	...
	•	create new record -> created_at == last_seen_at == now
	•	upsert existing with later now -> created_at unchanged, last_seen_at updated
	•	github_flow_available field preserved from input
