...
- Repo-aware commands such as `agency task start`, `agency worktree create`, and `agency agent start` should accept explicit `--repo` selectors from any cwd. When `--repo` is omitted, they should resolve the repo in this order: current directory, active context, then error.
