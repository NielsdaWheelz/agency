...
func FormatStdinMessage(runner, prompt string) ([]byte, error) {
	switch runner {
	case runners.RunnerAmp, runners.RunnerDroid:
...
// claudeStyleMessage is the JSONL format shared by Amp and Droid.
// Both use the same {"type":"user","message":{...}} envelope with content blocks.
