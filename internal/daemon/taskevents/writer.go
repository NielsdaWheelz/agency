...
// Appender appends task-scoped events.
type Appender interface {
	Append(eventsPath, taskID, kind string, data map[string]any, opts AppendOptions) (AppendResult, error)
}

// Writer appends task events with process-wide locking per events path.
type Writer struct {
...
// Append appends one task event under a single sequencing authority.
func (w *Writer) Append(eventsPath, taskID, kind string, data map[string]any, opts AppendOptions) (AppendResult, error) {
	if eventsPath == "" {
		return AppendResult{}, fmt.Errorf("events path is required")
	}
	if taskID == "" {
		return AppendResult{}, fmt.Errorf("task id is required")
	}
	if kind == "" {
