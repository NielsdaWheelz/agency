...
// Appender appends worktree-scoped events.
type Appender interface {
	Append(eventsPath, worktreeID, kind string, data map[string]any, opts AppendOptions) (AppendResult, error)
}

// Writer appends worktree events with process-wide in-memory locking per
// events path.
type Writer struct {
...
// Append appends one worktree event under a single sequencing authority.
func (w *Writer) Append(eventsPath, worktreeID, kind string, data map[string]any, opts AppendOptions) (AppendResult, error) {
	if eventsPath == "" {
		return AppendResult{}, fmt.Errorf("events path is required")
	}
	if worktreeID == "" {
		return AppendResult{}, fmt.Errorf("worktree id is required")
	}
	if kind == "" {
