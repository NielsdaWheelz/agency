...
func decodeRequestEnv(data []byte, allowedKeys map[string]bool) (map[string]string, bool, error) {
	...
	if err := json.Unmarshal(rawEnv, &rawValues); err != nil {
		return nil, true, fmt.Errorf("env must be an object")
	}
	if rawValues == nil {
		return nil, true, fmt.Errorf("env must be an object")
	}
	...
	for key, rawValue := range rawValues {
		if key == "" || strings.Contains(key, "=") || strings.ContainsRune(key, '\x00') {
			return nil, true, fmt.Errorf("env keys must be non-empty and must not contain '=' or NUL")
		}
		if bytes.Equal(bytes.TrimSpace(rawValue), []byte("null")) {
			return nil, true, fmt.Errorf("env.%s must be a string", key)
		}
		var value string
		...
		if strings.ContainsRune(value, '\x00') {
			return nil, true, fmt.Errorf("env.%s must not contain NUL", key)
		}
		env[key] = value
	}
	return env, true, nil
