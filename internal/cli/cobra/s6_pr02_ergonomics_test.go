...
func TestS6PR02_HighTrafficFlagAliases(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "agent merge repo", newCmd: newAgentMergeCmd, flagName: "repo", shorthand: "r"},
		{name: "agent merge json", newCmd: newAgentMergeCmd, flagName: "json", shorthand: "j"},
		{name: "agent merge yes", newCmd: newAgentMergeCmd, flagName: "yes", shorthand: "y"},
