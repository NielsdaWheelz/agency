...
func TestWatchActionDelegates_UseCanonicalCommandOptions(t *testing.T) {
	...
	delegates := newWatchActionDelegates(exec.NewRealRunner(), fs.NewRealFS(), "/tmp/repo", io.Discard, io.Discard)
	...
	delegates.prSyncFn = func(ctx context.Context, cr exec.CommandRunner, fsys fs.FS, cwd string, opts AgentPRSyncOpts, stdout, stderr io.Writer) error {
	...
	require.NoError(t, delegates.PRSync(context.Background(), "inv-3", "repo-3"))
	...
	assert.Equal(t, AgentPRSyncOpts{
		InvocationRef: "inv-3",
		RepoFlag:      "repo-3",
	}, gotPRSync.opts)
}
